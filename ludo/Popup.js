...
/*
3 Passo: Escutar o botão para que quando apertado exiba um enunciado de questão

Ideia: é possível usar objeto JSON
*/
//Selecionando elementos

const botaoRolar = document.querySelector(".dado-botao")
...
function definirDificuldadeTextoCor(){
    popupDificuldade.textContent = this.dificuldade;
...
//Quando clicar vai adicionar uma pergunta ao enunciado

//
function escutarBotaoRolar(){
    botaoRolar.addEventListener("click", executarSorteio);
}

function escutarPopupFechar(){
    popupFechar.addEventListener("click", ocultarPopup)

}



function escutarBotaoResposta(){
    botaoAdmitirResposta.addEventListener("click", validarResposta);
}
//Quando apertar no botão de admitir
function validarResposta() {
    acerto = "false"
    console.log("Clicou botáo")
    ...
    if(respostaUsuario === resposta){
        acerto = true;
        alert("Você acertou. Anda duas casas")
    } else{ //Se errou
        acerto = false;
        alert("Você errou. Paga uma prenda.")
    }
    inputResposta.value = ""
}

//Ideia: colocar dados vindo de um objeto JSON
// escutarBotaoRolar();
// escutarPopupFechar();
